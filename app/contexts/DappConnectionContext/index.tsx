...
export const DappConnectionContextProvider = ({
  ...
  const handleContactUpdateRequest = useCallback(
    (payload: JsonRpcRequest, meta: PeerMetadata) => {
      const eventType = RPC_EVENT.UPDATE_CONTACT
      const { params } = payload

      const contact = parseContact(params)

      if (!contact) {
        rejectCall(payload.id, 'Contact is invalid')
        return
      }

      if (!isContactExisted(contacts, contact.id)) {
        rejectCall(payload.id, 'Contact does not exist')
        return
      }
      ...
      Logger.info(`received CALL request, created ${eventType} event`)
    },
    [contacts]
  )

  const handleContactCreateRequest = useCallback(
    (payload: JsonRpcRequest, meta: PeerMetadata) => {
      const eventType = RPC_EVENT.CREATE_CONTACT
      const { params } = payload
  ...
  const handleContactRemoveRequest = useCallback(
    (payload: JsonRpcRequest, meta: PeerMetadata) => {
      const eventType = RPC_EVENT.REMOVE_CONTACT
      const { params } = payload
      const contactId = params?.[0]?.id

      if (!isString(contactId)) {
        rejectCall(payload.id, 'Contact ID is invalid')
        return
      }
