...
export default class {
  ...
  constructor() {
    ...
    this.tokenName = this.token.pipe(map(token => token?.name || ''));
    this.tokenSymbol = this.token.pipe(map(token => token?.symbol || ''));
    this.tokenDecimals = this.token.pipe(
      map(token => token?.decimals.toString() || ''),
    );
    this.addTokenBtnDisabled = this.token.pipe(map(token => token === null));
  }
