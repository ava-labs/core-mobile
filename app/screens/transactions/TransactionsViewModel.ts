...
export default class {
  ...
  private getBaseItem(value: HistoryItemType): HistoryItem {
    ...
    historyItem.explorerUrl =
      'https://explorer.avax-test.network/tx/' + value.id;
  ...
  private getTrxItem(value: HistoryItemType): HistoryItem {
    ...
    if ('tokens' in value) {
      ...
      let sentTokens = (value.tokens as iHistoryBaseTxTokens)
        .sent as iHistoryBaseTxTokensSent;
  ...
  private getExportItem(value: HistoryItemType): HistoryItem {
    ...
    if ('amount' in value) {
      historyItem.amount =
        '-' + Utils.bnToLocaleString(value.amount.add(value.fee), 9) + ' AVAX';
    }
    return historyItem;
  }
}
