...
export default class {
  ...
  destinationChain: BehaviorSubject<Chain> = new BehaviorSubject<Chain>(
    Chain.P,
  );
  ...
  getChainRenderItems(chains: Chain[]): ChainRenderItem[] {
    return chains.map(
      chain => new ChainRenderItem(chain, this.getChainString(chain)),
    );
  }
  ...
  makeTransfer(
    ...
    const exportOp: Observable<string> = this.getExportOpForSrcDestPair(
      srcChain,
      bnAmount,
      destChain,
    );
    const importOp: Observable<string> = this.getImportOpForSrcDestPair(
      destChain,
      srcChain,
    );
  ...
  private setLoaderVisibilityAndMsg(
    index: number,
    srcChain: Chain,
    destChain: Chain,
  ) {
  ...
  private getImportOpForSrcDestPair(destChain: Chain, srcChain: Chain) {
    ...
    switch (destChain) {
      case Chain.X:
        importOp = defer(() =>
          this.wallet.value.importX(<'P' | 'C'>srcChain.toString()),
        );
        break;
      case Chain.P:
  ...
  private getExportOpForSrcDestPair(
    srcChain: Chain,
    bnAmount: BN,
    destChain: Chain,
  ) {
    ...
    switch (srcChain) {
      case Chain.X:
        exportOp = defer(() =>
          this.wallet.value.exportXChain(
            bnAmount,
            <'P' | 'C'>destChain.toString(),
          ),
        );
        break;
      case Chain.P:
