#!/bin/bash

# AWS profile name
PROFILE_NAME="sso"

# Function to check if an AWS profile exists
awsConfigurationExists() {
    local profile_name="$1"
    local profile_status=$( (aws configure --profile "$profile_name" list) 2>&1)

    if [[ $profile_status == *'could not be found'* ]]; then
        return 1
    else
        return 0
    fi
}

# Function to retrieve secret by ID from AWS Secrets Manager
getSecretFromAWS() {
    local secret_id="$1"
    aws --profile $PROFILE_NAME secretsmanager get-secret-value --secret-id "$secret_id" | grep SecretString | sed 's/.*"SecretString": "\(.*\)".*/\1/'
}

# Function to check if AWS session is still valid
ensureAwsSession() {
    # Check if profile exists
    if ! awsConfigurationExists "$PROFILE_NAME"; then
        echo "Profile '$PROFILE_NAME' does not exist. Please create one first!"
        aws configure sso --profile "$PROFILE_NAME"
    fi

    # Check if session is valid (tolerate failure so an expired session
    # falls through to the login below even when the caller uses set -e)
    ACCOUNT=$(aws --profile "$PROFILE_NAME" sts get-caller-identity --query "Account" --output text 2>/dev/null || true)

    # Account is valid if account is a 12 digit number
    if [[ ${#ACCOUNT} -ne 12 ]]; then
        echo "Logging in with profile '$PROFILE_NAME'"
        aws sso login --profile "$PROFILE_NAME"
    fi
}
