...
| --- | --- | --- |
| `activate-build-cache-for-react-native@1` | `bitrise.yml`, in every `_build-*` workflow, before the first Gradle/Xcode step | Bitrise Build Cache. Remote Gradle task cache + **ccache** for the C++ native modules (Android) and the Xcode compilation cache (iOS) |
...
**Gradle dependency caching:** `restore-gradle-cache` / `save-gradle-cache` persisted *dependencies* (jars, modules, wrapper), which Build Cache does **not** replace. Removing them means Gradle re-resolves dependencies each build — measured at **28 s warm / 41 s cold** from distribution download to first task, roughly 4% of build time. The activation step configures Bitrise Maven mirrors (`repository-manager.services.bitrise.io`) automatically, which is what keeps this to seconds. Re-adding the pair alongside Build Cache is untested; Bitrise's FAQ advises against running standalone cache steps beside the React Native step.
...
- **Workflows DO share ccache entries.** ccache keys on compilation inputs (source, compiler, flags), not on workflow. `_build-android` (aab, 2 ABIs) reuses objects built by `_build-android-internal-for-testing` (apk, 3 ABIs) because its ABIs are a subset — measured: the release workflow's first warm run hit 925/925 (100%) with `Uploaded: 0 B`, meaning every object was already present from E2E runs. Gradle task outputs behave the same way. **Region is the thing that partitions the cache, not workflow.**
