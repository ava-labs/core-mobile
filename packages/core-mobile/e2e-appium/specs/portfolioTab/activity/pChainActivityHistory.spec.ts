...
describe('Activity history', () => {
  ...
  it('[P-Chain Send] verify send history on activity tab', async () => {
    await portfolio.verifyXPSendOnActivityTab('0.01 AVAX sent', common.pChain)
  })

  it('[P-Chain Receive] verify receive history on activity tab', async () => {
    await settingsPage.switchAccount(settings.account2)
    await portfolio.verifyXPSendOnActivityTab('0.01 AVAX received')
  })

  it('[P-Chain Receive] verify receive history on the token detail', async () => {
