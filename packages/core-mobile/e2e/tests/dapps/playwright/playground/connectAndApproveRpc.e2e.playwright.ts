...
describe('Playground', () => {
  ...
  it('eth_sendTransaction', async () => {
    await actions.waitForElementNoSync(popup.approveTransactionTitle, 30000)
    await actions.tap(popup.approveBtn)
  })

  it('eth_signTypedData_v3', async () => {
    await actions.waitForElementNoSync(popup.signMessage, 30000)
    await actions.tap(popup.approveBtn)
  })
  ...
  it('eth_sign', async () => {
    await actions.waitForElementNoSync(popup.proceedAnyway, 30000)
    await actions.tap(popup.proceedAnyway)
    await actions.waitForElementNoSync(popup.approveBtn, 30000)
