...
const mockListSeries =
  predictionMarketClient.markets.listSeries as jest.MockedFunction<
    typeof predictionMarketClient.markets.listSeries
  >

const mockSeries = [
  { ticker: 'POLITICS', title: 'Politics', category: 'Politics', kycRequired: false },
  { ticker: 'FINANCE', title: 'Finance', category: 'Finance', kycRequired: false }
]
...
describe('useMarketSeries', () => {
  ...
  it('returns series array when data is available', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    mockUseQuery.mockReturnValue({ data: mockSeries, isLoading: false, error: null } as any)
    const { result } = renderHook(() => useMarketSeries())
