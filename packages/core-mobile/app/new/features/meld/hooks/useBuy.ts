...
import { isAllowedLimitedLocalToken, isLimitedMode } from 'utils/limitedMode'
...
export const useBuy = (): {
  ...
  isAvaxCBuyable: () => boolean
  isUsdcBuyable: () => boolean
} => {
  ...
  const isBuyable = useCallback(
    (token?: LocalTokenWithBalance, address?: string) => {
      ...
      // Limited mode: gate by the same six-token allowlist used by swap.
      // Identifies the token by chain + address (LocalTokenWithBalance shape)
      // since `internalId` is not reliably populated for all chains.
      if (isLimitedMode && token && !isAllowedLimitedLocalToken(token))
        return false
      return true
    },
  ...
  const isAvaxCBuyable = useCallback(() => {
    return avax !== undefined
  }, [avax])

  const isUsdcBuyable = useCallback(() => {
