...
export const useKeystoneDeprecation = (): {
  ...
  const shouldWarnForWalletType = useCallback(
    (walletType: WalletType): boolean => walletType === WalletType.KEYSTONE,
    []
  )
  ...
  return { shouldWarnForWalletType, openDeprecationInfo }
}
...
export const useIsActiveWalletKeystoneDeprecated = (): boolean => {
  ...
  return activeWalletType === WalletType.KEYSTONE
}
