...
export function useUserBalances(): {
  ...
  const accountsArray = useMemo(() => Object.values(allAccounts), [allAccounts])

  const isNotReady = accountsArray.length === 0 || enabledNetworks.length === 0
  ...
  }, [accountsArray, enabledNetworks, currency, isNotReady])
