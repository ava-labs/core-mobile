...
import { useTokensWithBalanceForAccountFromSource } from './useTokensWithBalanceForAccountFromSource'

/**
 * Returns the total balance (in fiat currency) for a given account,
 * filtered by token visibility and enabled networks.
 */
export function useBalanceTotalInCurrencyForAccountFromSource({
  account,
  sourceData
}: {
  account?: Account
  ...
  const tokens = useTokensWithBalanceForAccountFromSource({
    account,
    sourceData
  })
