...
describe('RecurringSchedulesService', () => {
    ...
    fetchMock.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          address: '0xabc',
          count: 1,
          orders: [{ orderId: '0xdead', status: 'active' }]
        }),
        { status: 200 }
      )
    )
  ...
  it('cancel treats 400 "order not cancellable" as a typed error', async () => {
    ...
    await expect(
      service.cancel({ orderId: '0xdead', address: '0xabc' })
  ...
  it('list treats 401 as unauthorized error', async () => {
    ...
    await expect(service.list({ address: '0xABC' })).rejects.toMatchObject({
      kind: 'unauthorized'
    })
  })
  ...
  it('list throws generic Error for non-401 non-2xx', async () => {
    ...
    await expect(service.list({ address: '0xABC' })).rejects.toThrow(
      'recurring/orders 500'
    )
  })
