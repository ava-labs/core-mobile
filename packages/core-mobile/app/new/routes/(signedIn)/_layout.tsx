  ...
  return (
    <WatchlistQueriesProvider>
      <CollectiblesProvider>
        <LedgerSetupProvider>
          <PerpsProvider>
            {/* CP-14918 TEMP PROBE: per-commit JS render time for the whole
                (signedIn) navigator tree — tabs + every registered modal
                Stack.Screen. tokenDetail's first commit is a 'nested-update'
                that starts here, never a 'mount' local to the screen, so
                this boundary is what actually captures the 764-948ms
                route-mount cost measured above every other Profiler in the
                app. Compare against 'tokenDetailRouteStack' (per-route
                nested Stack, tokenDetail/_layout.tsx) to see how much of
                this commit's duration is the surrounding navigator tree
                re-rendering vs. the tokenDetail route's own subtree. */}
            <React.Profiler id="signedInStack" onRender={perfRenderProfile}>
              <Stack
