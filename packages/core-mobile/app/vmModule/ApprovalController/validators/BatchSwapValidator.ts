...
export const batchSwapValidator: ApprovalValidator = {
  canHandle: (request: RpcRequest): boolean => {
    ...
    if ((request.method as string) !== RpcMethod.ETH_SEND_TRANSACTION_BATCH)
      return false
    const ctx = readAutoApproveContext(request)
  ...
  validate: async (
    ...
    try {
      ...
      Logger.warn('[BatchSwapValidator] validation failed; falling back', err)
    ...
    if (result.isValid) {
      ...
      Logger.info('[BatchSwapValidator] fell back to manual', {
        code: result.code,
