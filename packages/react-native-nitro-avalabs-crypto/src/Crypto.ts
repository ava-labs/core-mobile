...
function hexLikeToArrayBuffer(input: string | ArrayBuffer | Uint8Array): ArrayBuffer {
...
export function getPublicKey(
  ...
  if (typeof secretKey === 'bigint') {
    ...
    con.log('[Crypto] Secret is buffer-like', secretKey instanceof Uint8Array, secretKey instanceof ArrayBuffer);
  ...
  if (ab && typeof NativeCrypto.getPublicKeyFromArrayBuffer === 'function') {
  ...
  } else if (hex && typeof NativeCrypto.getPublicKeyFromArrayBuffer === 'function') {
    con.log('[Crypto] Calling NativeCrypto.getPublicKeyFromArrayBuffer');
    outBuf = NativeCrypto.getPublicKeyFromArrayBuffer(hexToArrayBuffer(hex), isCompressed);
    ...
    throw new Error('Native Crypto hybrid does not expose the expected methods.');
  }

  con.log('[Crypto] Native call succeeded, result byteLength:', outBuf?.byteLength);
...
export const getPublicKeyFromArrayBuffer = NativeCrypto.getPublicKeyFromArrayBuffer.bind(NativeCrypto);
export const getPublicKeyFromString = NativeCrypto.getPublicKeyFromString.bind(NativeCrypto);
...
function compact64ToDer(sig64: Uint8Array): Uint8Array {
  if (sig64.length !== 64) throw new TypeError('compact ECDSA signature must be 64 bytes');
...
export function sign(
  ...
  if (msgAB.byteLength !== 32) throw new TypeError('ECDSA message must be 32 bytes');
...
function parseDerEcdsa(sig: Uint8Array): { r: Uint8Array; s: Uint8Array } {
  if (sig.length < 8 || sig[0] !== 0x30) throw new TypeError('Invalid DER: no SEQ');
  let p = 1;
  let len = sig[p++];
  if (len & 0x80) {
    const n = len & 0x7f;
  ...
  let r = sig.subarray(p, p + rLen); p += rLen;
...
function derToCompact64(sigDer: Uint8Array): Uint8Array {
  ...
  if (r.length > 32 || s.length > 32) throw new TypeError('Invalid DER: r/s too long');
...
export function verify(
  ...
  if (!isDerSignature(sigU8)) {
    if (sigU8.length === 64) {
      ...
      con.log('[Crypto] verify: non-DER signature with unexpected length', sigU8.length);
    }
  }
...
export function signSchnorr(
  ...
  const msgAB = ensure32('Schnorr messageHash', hexLikeToArrayBuffer(messageHash as any));
  const skAB  = ensure32('Schnorr secretKey',   hexLikeToArrayBuffer(secretKey as any));
  ...
  if (auxRand === undefined) {
    ...
    con.log('[Crypto] signSchnorr: auxRand provided, byteLength=', (auxAB as ArrayBuffer).byteLength);
  }
...
export function verifySchnorr(
  ...
  if (msgAB.byteLength !== 32) throw new TypeError('Schnorr messageHash must be 32 bytes');
  if (sigAB.byteLength !== 64) throw new TypeError('Schnorr signature must be 64 bytes');
