...
export function useBalances(w: WalletProvider) {
  ...
  const [newBalanceX, setNewBalanceX] = useState<WalletBalanceX>(wallet.getBalanceX())
  ...
  const balanceXtoBN = (newBalanceX?: WalletBalanceX): BN => {
    if (newBalanceX === undefined) return new BN(0)
    return newBalanceX['U8iRqJoiJm8xZHAacmvYyZVwqQx6uDNtQeP3CQ6fcgQk3JqnK'].unlocked
  }
  const balancePToBN = (newBalanceP?: AssetBalanceP): BN => {
    if (newBalanceP === undefined) return new BN(0)
    return newBalanceP.unlocked
  }

  const balanceCToBN = async (newBalanceC?: BN) => {
    await wallet.evmWallet.updateBalance()
    if (newBalanceC === undefined) return new BN(0)
    return newBalanceC
  }
  ...
  const getTotalAmount = (balanceX: BN, balanceP: BN, balanceC: BN, stake: BN) => {
  ...
  useEffect(() => {
    Promise.all([balanceCToBN(newBalanceC), fetchStake(wallet)])
      .then(([balC, stake]) => {
        setAvailableTotal(getTotalAmount(balanceXtoBN(newBalanceX), balancePToBN(newBalanceP), balC, stake))
      })
      .catch(reason => console.error(reason))
