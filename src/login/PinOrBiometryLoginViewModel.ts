...
import {PinKeys} from "../onboarding/PinKey"
...
export function usePinOrBiometryLogin(): [string, string, DotView[], (pinKey: PinKeys) => void, string | undefined, () => Observable<WalletLoadingResults>] {
  ...
  const promptForWalletLoadingIfExists = (): Observable<WalletLoadingResults> => {
    return timer(100, asyncScheduler).pipe( //timer is here to give UI opportunity to draw everything
